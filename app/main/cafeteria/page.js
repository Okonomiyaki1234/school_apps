	...
	return (
		<>
			...
			<main style={{ padding: "2rem", paddingTop: "3.5rem" }}>
				...
				{loading ? (
					<div style={{ textAlign: 'center', color: '#888', margin: '2rem 0' }}>読み込み中...</div>
				) : error ? (
					<div style={{ textAlign: 'center', color: '#b00', margin: '2rem 0' }}>データ取得エラー: {error}</div>
				) : (
					<div
						style={{
							display: "flex",
						...
						{categories.map((cat) => {
							...
							return (
								...
								>
									...
									>
										{items.length === 0 ? (
											<div style={{ color: '#aaa', textAlign: 'center', padding: '1.2em 0' }}>該当なし</div>
										) : (
											items.map((item) => (
												<div
													key={item.id}
